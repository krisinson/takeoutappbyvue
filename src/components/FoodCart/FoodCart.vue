<template>
 <div>
    ...
    <div class="content2">
      <div class="content-left">
        <div class="logo-wrapper">
          <div class="logo" @click="ToggleShopCart">
            <i class="iconfont icon-gouwuche " :class="{highlight:totalCount}" ></i>
          </div>
          <div class="num" v-show="totalCount">{{totalCount}}</div>
        </div>
        <div class="price highlight" v-show="totalPrice">￥{{totalPrice}}</div>
   ...
   <transition name="move">
    <div class="shopcart-list" v-show="isShowShopCart" >
      ...
      <div class="list-content">
        <ul>
          <li class="food" v-for="(food,index) in shopsCart" :key="index">
  ...
  <div class="list-mask" @click="isShowShopCart=false" v-show="isShowShopCart"></div>
</div>
</template>
  ...
  export default {
    ...
    computed:{
      ...
      payClass(){
        ...
        return this.totalPrice>this.info.minPrice?'enough':'not-enought'
      },
      payText(){
        ...
        if(totalPrice===0){
          return `还差${info.minPrice}元起送`
        }else if(totalPrice>0&&totalPrice<info.minPrice){
          return `还差${info.minPrice-totalPrice}元配送`
        }else{
          return '去结算'
        }
      }
    ...
    methods:{
      ...
      clearCart(){
        ...
        .then(
          actionAgree =>  this.$store.commit(CLEAR_CART),
    ...
    watch:{
      totalCount(newValue){
        ...
        !!!newValue && (this.isShowShopCart = false)
      }
    }
  }
</script>
